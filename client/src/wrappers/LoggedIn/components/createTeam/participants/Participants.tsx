          ...
          {candidates.length > 0 ? (
            <>
              <Grid container spacing={4} className={classes.partiHeader}>
                ...
                <Grid item xs={2} md={3}>
                  <ListItemText secondary={t('Cost')} />
                </Grid>
                <Grid item xs={2} md={3}>
                  <ListItemText secondary={t('Prospects')} />
                </Grid>
              </Grid>
              {candidates.map((candidate, idx) => (
                <List key={uuidv4()} component="nav" aria-label="main mailbox folders">
                  <ListItem button>
                    <ListItemIcon>{noEdit ? <LibraryAddCheckIcon /> : <InboxIcon />}</ListItemIcon>
                      ...
                      <Grid item xs={1} md={1}>
                        {!noEdit && (
                          <Fab
                            color="primary"
                            aria-label="delete"
                            onClick={() => removeCandidateHandler(idx)}
                          >
                            <DeleteForeverIcon />
                          </Fab>
                        )}
