...
interface PostContentProps {
  ...
  pushMessageToSnackbar: (message: { text: string }) => void
}
const PostContent: React.FC<PostContentProps> = (props) => {
  const { pushMessageToSnackbar, setPosts, posts, openAddPostModal, classes } = props
  ...
  const deletePost = useCallback(
    (params) => {
      ...
      setTimeout(() => {
        const _posts = [...posts]
        const index = _posts.find((element) => element.id === params.id)
  ...
  return (
    <Paper>
      <Toolbar className={classes.toolbar}>
        ...
        <Button variant="contained" color="secondary" onClick={openAddPostModal} disableElevation>
          Add Post
        </Button>
      </Toolbar>
      <Divider />
