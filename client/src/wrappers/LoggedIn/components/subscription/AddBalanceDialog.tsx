...
import { AddBalanceDialogProps } from './LazyLoadAddBalanceDialog'

const stripePromise = loadStripe('pk_test_6pRNASCoBOKtIshFeQd4XMUh')

const paymentOptions = ['Credit Card', 'SEPA Direct Debit']

const AddBalanceDialog = withTheme((props: AddBalanceDialogProps) => {
  ...
  return (
    <FormDialog
      ...
      onFormSubmit={async (event) => {
        ...
        setLoading(true)
        const { error } = await stripe.createPaymentMethod(getStripePaymentInfo())
        if (error) {
...
function Wrapper(props) {
  ...
  return (
    <Elements stripe={stripePromise}>
      {open && <AddBalanceDialog open={open} onClose={onClose} onSuccess={onSuccess} />}
    </Elements>
  )
}
